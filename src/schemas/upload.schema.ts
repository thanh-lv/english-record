...
export const ALLOWED_MEDIA_MIME_TYPES = [...ALLOWED_IMAGE_MIME_TYPES, ...ALLOWED_AUDIO_MIME_TYPES];
...
export function createFileSchema(options: {
  ...
  return z
    .custom<File | Blob>(file => file instanceof Blob || file instanceof File, {
    ...
    .refine(file => file.size <= maxBytes, {
      message:
        options.sizeErrorMessage || `Dung lượng tệp vượt quá giới hạn cho phép (${maxSizeMb}MB).`,
    });
}
