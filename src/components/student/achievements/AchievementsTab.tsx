...
export function AchievementsTab({ totalNumbers, completedNumbers }: AchievementsTabProps) {
  const { t } = useLanguage();
  const pct =
    totalNumbers.length > 0 ? Math.round((completedNumbers.length / totalNumbers.length) * 100) : 0;
  ...
  return (
    <div className="bg-white/70 backdrop-blur-sm p-5 sm:p-8 rounded-lg border-3 border-white shadow-md space-y-5">
      <div className="text-center space-y-1">
        ...
        <p className="text-slate-500 font-bold text-sm">{t.achievements.subtitle}</p>
      </div>
      ...
      <div className="bg-amber-50 border-2 border-amber-200 rounded-lg p-4 flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="font-black text-amber-800 text-sm">
            {completedNumbers.length}/{totalNumbers.length} {t.achievements.lessons}
          </p>
      ...
      {completedNumbers.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-lg border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-400 font-bold text-sm">{t.achievements.empty}</p>
          <p className="text-slate-400 text-xs mt-1">{t.achievements.emptyHint}</p>
        </div>
      ) : (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4">
          {completedNumbers
            ...
            .map(num => (
              ...
              >
                <span className="text-3xl drop-shadow-md">{getPrizeForTopic(num)}</span>
