...
export function TeacherSidebar() {
  ...
  const activeTab = (location.pathname.split('/')[2] || 'recordings') as TeacherTab;
      ...
      >
        <span
          className={`shrink-0 transition-transform duration-200 group-hover:scale-110 ${
            active ? theme.iconActiveClass : 'text-slate-400 group-hover:text-slate-600'
          }`}
        >
          {item.icon}
        </span>
        ...
        {active && (
          <span className={`ml-auto w-2 h-2 rounded-full shadow-xs shrink-0 ${theme.dotClass}`} />
        )}
      </Link>
    );
  };
    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-64 shrink-0 bg-white/95 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200/80 sticky top-[84px] max-h-[calc(100vh-100px)] overflow-hidden">
        <div className="px-5 pt-5 pb-3 border-b border-slate-100 flex items-center justify-between">
          <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Menu</p>
